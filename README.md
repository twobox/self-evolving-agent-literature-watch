...
首次运行会根据 `STATE.md` 启动基线阶段并建立初始候选池；后续运行在执行增量监控的同时，继续补全尚未覆盖的基线方向。需要指定补漏方向、仅检查重要更新或只讨论方案时，参见 [`PROMPTS.md`](PROMPTS.md)。
...
仓库初始化完成后，第一次正式执行应启动基线阶段，围绕一个有限范围检查近三年的相关论文，在同一次事务中建立首份运行报告和初始候选池，并在 `STATE.md` 中记录实际覆盖进度。基线阶段通过后续多次运行持续推进，直至主要方向达到足够覆盖。
